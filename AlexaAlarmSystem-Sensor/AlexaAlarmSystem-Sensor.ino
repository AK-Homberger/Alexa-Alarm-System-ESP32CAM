...
// Extended version with additional CCS811 air quality sensor
//
...
#define BAD_AIR 2300          // Set bad air level for LED ON and Alexa notification (+200)
...
#define D_MOVE_WAIT 60000*60  // 1 hour wait time between disarmed movement URL triggers

#define On true
#define Off false
#define LED_BUILTIN 4

// CCS811 requires I2C. I2C for ESP32-CAM requires special ports
#define I2C_SDA 14            // Set SDA to port 14 (available on ESP32-CAM when SD is not used)
#define I2C_SCL 15            // Set SCL to port 15 (available on ESP32-CAM when SD is not used)
...
#define SILENT_ALARM On                 // On means no internal LED blink and no Alexa notification sound
...
// Trigger URLs: 1. Alexa alarm routine, 2. Ping notification sound, 3. Bad air quality trigger
const char* URL[] PROGMEM = {"https://www.virtualsmarthome.xyz/url_routine_trigger/1..",
                             "https://www.virtualsmarthome.xyz/url_routine_trigger/2..",
                             "https://www.virtualsmarthome.xyz/url_routine_trigger/3.."
                            };
...
// CCS811 sensor data
Adafruit_CCS811 ccs;                // CCS811 air quality sensor object
uint16_t eCO2 = 0;                  // eCO2 value (VOC equivalent to CO2)
TwoWire I2CSensors = TwoWire(0);    // New I2C object (to be initialised with changed ports)
...
void setup() {
  ...
  server.on("/set_baseline", handleBaseline); // Handle saet CCS811 baseline
...
//*******************************************************************************
// Check WiFi and try WiFi restart if connection is lost. Restart ESP if not successful
//
...
//*******************************************************************************
// Send sensor, alarm state and air quality as JSON data
//
void handleGetData() {
  char Text[40];
  String JSON_Text;
...
void handleUptime() {
  ...
  char text[120];
  struct tm timeinfo;
  uint16_t baseline, stored_baseline;

  baseline = ccs.getBaseline();
  stored_baseline =  (EEPROM.read(1) << 8) + EEPROM.read(2);

  getLocalTime(&timeinfo);
  strftime(time_str, sizeof(time_str), "%T", &timeinfo);

  snprintf(text, sizeof(text), "Time: %s\nUptime: %d (hours)\nFree Heap: %d\nSingle=%d Double=%d\nMax FPS=%d\nBaseline=0x%x, Stored=0x%x\n",
           time_str, millis() / 3600000, ESP.getFreeHeap(), single_counter, double_counter, max_fps, baseline, stored_baseline);
...
//*****************************************************************************
// Handle set_baseline request baseline for CCS811
//
void handleBaseline() {
  char text[20];
  uint16_t baseline;

  baseline = ccs.getBaseline();     // Get current baseline from CCS811 sensor
  snprintf(text, sizeof(text), "Baseline: %x\n", baseline);
  EEPROM.write(1, baseline >> 8);   // Write new baseline to flash
  EEPROM.write(2, baseline  & 0xFF);
  EEPROM.commit();
...
void Handle_PIR_Sensor(void) {
  ...
  if (!SILENT_ALARM) ledcWrite(2, PIR_On * 10); // Show state on internal LED if not SILENT_ALARM
...
//*******************************************************************************
// Read CCS811 air quality sensor once per second
//
void read_ccs(void) {
...
void restore_baseline(void) {
  ...
  if (millis() > (20 * 60000) && !written) {

    uint16_t baseline =  (EEPROM.read(1) << 8) + EEPROM.read(2);
