...
Then a VPN connection to the own network is a bit more difficult. But you can use an [OpenVPN](https://openvpn.net/) server on a [Raspberry](https://www.ionos.de/digitalguide/server/konfiguration/vpn-server-einrichten-via-raspberry-pi-und-openvpn/) device in your local network together with an commercial portmapper service (e.g. [feste-ip.net](https://www.feste-ip.net/dslite-ipv6-portmapper/allgemeine-informationen/)). On the phone you can the install one of the available OpenVPN client apps.
